module.exports.deploy = async function (inputFile, gasPriceGwei, rpcUrl, dontSendTx) {
    ...
    const tokenContractPath = path.join(__dirname, "/contracts/");
    ...
    const compilationInput = {
        ...
        "WGhost.sol" : fs.readFileSync(tokenContractPath + 'WGhost.sol', 'utf8')
    };
    ...
    async function main() {
        ...
        [tokenAddress, tokenContract] = await deployContract(output, "WGhost.sol:WGhost", []);
        ...
        let custodianGhostDepositAddress = web3.utils.fromAscii("test");
        console.log("factoryContract.methods.setCustodianGhostDepositAddress: " + accountMerchantAddress + ", " + custodianGhostDepositAddress);
        await sendTx(factoryContract.methods.setCustodianGhostDepositAddress(accountMerchantAddress, custodianGhostDepositAddress), accountCustodian);

        nonce = await web3.eth.getTransactionCount(accountMerchantAddress);
        console.log("accountMerchantAddress nonce: " + nonce);
        let merchantGhostDepositAddress = "GE57B5SCkGVhFxDugko3quHxamPgkS8NxJ";
        console.log("factoryContract.methods.setMerchantGhostDepositAddress: " + merchantGhostDepositAddress);
        await sendTx(factoryContract.methods.setMerchantGhostDepositAddress(merchantGhostDepositAddress), accountMerchant);
